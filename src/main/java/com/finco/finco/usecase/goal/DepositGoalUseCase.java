...
public class DepositGoalUseCase {
    ...
    public Goal execute(Long goalId, IGoalDepositData goalDepositData) {
        ...
        BigDecimal totalBalanceInGoalsByAccount = goalAccountBalanceGateway.getTotalBalanceInGoalsByAccount(account.getId());
        if(totalBalanceInGoalsByAccount.add(goalDepositData.amount()).compareTo(account.getBalance()) > 0) {
