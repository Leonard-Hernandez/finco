...
public class WithDrawAccountUseCaseTest {
    ...
    @BeforeEach
    public void setUp() {
        withDrawAccountUseCase = new WithDrawAccountUseCase(accountGateway, authGateway);
        ...
        transactionData = new AccountTransactionData(withdrawAmount);
    }
    ...
    public void withdrawMoreThanBalanceShouldThrowException() {
        // Arrange
        transactionData = new AccountTransactionData(excessiveWithdrawAmount);
        when(accountGateway.findById(accountId)).thenReturn(Optional.of(testAccount));
        doNothing().when(authGateway).verifyOwnershipOrAdmin(userId);
