...
public class GetAllTransactionsByUserUseCaseTest {
    ...
    public void getAllTransactionsByUserSuccess() {
        // Arrange
        when(transactionGateway.findAllByUserId(userId, pageRequest)).thenReturn(pagedResult);
        doNothing().when(authGateway).verifyOwnershipOrAdmin(userId);

        // Act
        PagedResult<Transaction> result = getAllTransactionsByUserUseCase.execute(pageRequest, userId);
        ...
        verify(transactionGateway, times(1)).findAllByUserId(userId, pageRequest);
    }
    ...
    public void getEmptyTransactionsListWhenUserHasNoTransactions() {
        ...
        when(transactionGateway.findAllByUserId(userId, pageRequest)).thenReturn(emptyResult);
        doNothing().when(authGateway).verifyOwnershipOrAdmin(userId);
    ...
    public void getTransactionsWithoutPermissionShouldThrowException() {
        ...
        assertThrows(AccessDeniedBusinessException.class, () -> {
            getAllTransactionsByUserUseCase.execute(pageRequest, userId);
        });

        verify(authGateway, times(1)).verifyOwnershipOrAdmin(userId);
        verify(transactionGateway, never()).findAllByUserId(anyLong(), any());
    }
    ...
    public void getTransactionsWithPagination() {
        ...
        when(transactionGateway.findAllByUserId(userId, customPageRequest)).thenReturn(pagedResult);
        doNothing().when(authGateway).verifyOwnershipOrAdmin(userId);

        // Act
        PagedResult<Transaction> result = getAllTransactionsByUserUseCase.execute(customPageRequest, userId);

        // Assert
        assertNotNull(result);
        verify(transactionGateway, times(1)).findAllByUserId(userId, customPageRequest);
        verify(authGateway, times(1)).verifyOwnershipOrAdmin(userId);
    }
}
